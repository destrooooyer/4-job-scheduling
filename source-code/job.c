...
void do_stat(struct jobcmd statcmd)
{
	...
	*/
	...
	/* 打印信息头部 */
	sprintf(temp_str_out, "JOBID\tPID\tOWNER\tRUNTIME\tWAITTIME\tCREATTIME\t\tSTATE\n");
	if (current){
		char temp_str[1000] = { 0 };
		strcpy(timebuf, ctime(&(current->job->create_time)));
		timebuf[strlen(timebuf) - 1] = '\0';
		sprintf(temp_str, "%d\t%d\t%d\t%d\t%d\t%s\t%s\n",
			current->job->jid,
			current->job->pid,
			current->job->ownerid,
	...
	int temp_data_stat;
	if ((temp_data_stat = open("/tmp/temp_data_stat", O_WRONLY)) < 0)
		error_sys("opening failed");

	// 	for (int i = 0; i < 100; i++)
	// 		temp_str_out[i] = 'w';
	/*	char a[100] = "23434\n2342342\ndf\ndfgdf\n";*/
	/*	printf("%s", temp_str_out);*/
	if (write(temp_data_stat, temp_str_out, 10000) < 0)
		error_sys("stat write failed");

	close(temp_data_stat);

	//create("/tmp/temp_flag_stat", S_IREAD | S_IWRITE);

}

int main()
{
	remove("/tmp/temp_data_stat");
	if (mkfifo("/tmp/temp_data_stat", 0666) < 0)
